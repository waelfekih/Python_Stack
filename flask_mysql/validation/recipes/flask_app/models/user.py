...
class User: 
    ...
    def validate_register(data):
        ...
        if not EMAIL_REGEX.match(data['email']) :
            is_valid = False
            flash("Email format incorrect" , "register")
        ...
        if len(data['first_name']) < 2 :
            is_valid = False
            flash("First Name must be at least 2 characters length" , "register")

        if len(data['last_name']) < 2 :
            is_valid = False
            flash("Last Name must be at least 2 characters length" , "register")

        if int(len(data['password'])) < 8 :
            is_valid = False
            flash("Password should be at least 8 characters length" , "register")
